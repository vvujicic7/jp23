...
# Mjestima dodati kolonu brojstanovnika i u nju postaviti slučajne vrijednosti između 10000 i 100000 = RIJEŠENO
select * from mjesto;
update mjesto set brojstanovnika=10000 + (rand()*90000);

# odaberite podatke na način da ispis u Gridu izgleda npr: ovako:
# Grad: Osijek, 31000 (Županija:14) = RIJEŠENO
select naziv, postanskibroj, zupanija from mjesto;

# baza knjiznica
# 1. odaberite sve autore koji nemaju definiran datum rođenja = RIJEŠENO
select * from autor;
select * from autor where datumrodenja is null;

# 2. autora s prezimenom preišić ispraviti u Previšić = RIJEŠENO 
update autor set prezime='Previšić' where sifra=3;

# 3. kreirati tablicu grad (id,naziv). U tablicu autor dodati kolonu grad koja je vanjski ključ na grad(id) =RIJEŠENO
select * from autor;
...
# DZ razmišljati što raditi za završni rad - HOTEL